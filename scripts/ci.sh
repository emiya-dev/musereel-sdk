...
# conformance 代码使用 build tag，离线门禁只守它始终可编译且 vet 干净。
# 不加 `go test -tags conformance`：conformance_test.go 只接受真实 E14 compose
# 环境，缺 gateway、mTLS 或签名输入时必须显式失败；离线 ci 没有这些运行时输入。
